...
source "googlecompute" "consul-nomad" {
  ...
  image_name          = "consul-nomad-client-${local.timestamp}"
  image_family        = "consul-nomad-client"
  image_description   = "Consul ${var.consul_version} and Nomad ${var.nomad_version} Client"
  ...
  tags = ["packer", "consul-nomad-client"]
}

build {
  name = "consul-nomad-client"
  sources = ["source.googlecompute.consul-nomad"]
  
  hcp_packer_registry {
    bucket_name = "consul-nomad-client"
    description = "Consul ${var.consul_version} and Nomad ${var.nomad_version} Client Image"
    bucket_labels = {
